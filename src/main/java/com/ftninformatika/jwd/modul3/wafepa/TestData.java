...
private LocationService  locationSercvice;
	...
	public void init() {
		...
		apart1.setHost(hostService.one(1L).get());
		...
		apart2.setHost(hostService.one(2L).get());
		...
		Reservation reservation1=new Reservation(apart1,LocalDate.of(2020, 11, 18),5L,guestService.one(1L).get());
		reservation1.setReservationStatus(ReservationStatus.CREATED);
		Reservation reservation2=new Reservation(apart2,LocalDate.of(2020, 11, 22),3L,guestService.one(2L).get());
